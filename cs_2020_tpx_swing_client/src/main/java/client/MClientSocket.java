 ...
 *
 */
public class MClientSocket implements ISessionClient {

	private static final boolean LOG = true;

	// private IModel model;
	private String serverName = "-1";
	private int serverPort = IConstants.DEFAULT_PORT;
...
//	private IClientControler controler;
	...
	public MClientSocket() {
		// this.host_ = uri;
		keywords.add(IConstants.KW_CIRCLE + " ");
		keywords.add(IConstants.KW_OVAL + " ");
	...
	public void setHost(String uri, int port) {
		this.serverName = uri;
	...
	private void sendIdentification() {
		String ids = "";
		int[] id = controler.getIdentifiers();
		for (int i : id) {
	...
	private void clear() {
		...
		IModel model = controler.getModel();
		if (model != null) {
			synchronized (model) {
				model.clearFigures();
			}
	...
	private void viewRefresh(String figure) {
		controler.refreshFigure(figure);
	}
	...
	public void setMessageHandler(IClientControler wsClient) {
		this.controler = wsClient;
	}
	...
	public void jAddMessage_(String message) {
	...
	@Override
	public void setControler(IClientControler cc) {
		this.controler = cc;
	}
